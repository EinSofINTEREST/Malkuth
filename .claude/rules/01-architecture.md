...
5. **확장 상한 없는 그룹은 확장이 없다**: `spec.access.ceiling` 을 선언하지 않은 그룹의 멤버에게
...
**컨테이너 안의 검사는 강제 수단이 아니다** — 도구 목록, 호출자 쪽 allowlist, 컨테이너 env
는 편의이자 기본값일 뿐이다. 실시간으로 부여·회수할 수 있는 권한은 반드시 컨테이너 밖에서,
요청마다 판정한다.
