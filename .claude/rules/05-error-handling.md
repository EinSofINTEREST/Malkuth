...
RT_010:  이미 기동된 레플리카 — 중복 기동 거부 (상태 충돌, HTTP 409)
RT_011:  이미 진행 중인 이미지 빌드 — 중복 요청 거부 (상태 충돌, HTTP 409)
RT_012:  굽지 않았거나 빌드가 실패한 커스텀 이미지 — 배포 거부 (상태 충돌, HTTP 409)
...
GRAPH_006: Run 상태가 요청한 조작과 맞지 않음 (상태 충돌, HTTP 409)
