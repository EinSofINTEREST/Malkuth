...
**목록에 있는 자격 이름만 보낸다.** 선언이 모델 키 같은 다른 비밀을 `auth.token_env` 로 적으면 그 값 대신
`502`(`CFG_002`) 로 답한다. `MALKUTH_EGRESS_ALLOW_PLAINTEXT_UPSTREAM=true` 가 아니면 자격을 평문 `http` 로
보내지 않고, 사설 주소로 풀리는 서버는 여느 목적지처럼 `MALKUTH_EGRESS_PRIVATE_DESTINATIONS` 에 있어야 한다.

배포는 원격 MCP 자격을 에이전트 env 에 넣지 않는다. 검증이 해석할 수 있도록 평소처럼 스코프에
선언한다(`env_allowlist` 와 그룹 또는 global `secrets` 목록).

닿지 않는 것: stdio MCP 서버는 에이전트 컨테이너 안에서 돌기 때문에 도구 하나씩 판정하지 않는다 — 외부 효과는
프록시의 목적지 판정을 거치고, 로컬 효과는 실시간 통제 대상이 아니다. 사이드카 MCP 서버는 프록시를 거치지
않는다.
