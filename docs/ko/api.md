...
**강제는 지점별로 차례차례 들어온다.** 메모리는 지금 강제된다: 레지스트리 모드의 Memory Service 가
요청마다 묻는다 ([메모리 강제](#메모리-강제) 참조). 이그레스, 원격 MCP 도구, A2A 호출은 여기에
기록되지만 아직 강제되지 않으므로, 그 종류의 판정은 에이전트가 닿는 범위를 바꾸지 않는다.
...
2단계는 강제 지점이 갖춰진 자원 종류에만 적용된다 — 지금은 `memory`. 종류별 선언 판정은 그것을
