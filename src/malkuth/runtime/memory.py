...
않기 위해서다 (09 Access Enforcement). 별칭 해석(local > group > global)은
토큰이 담당하므로 어댑터는 그것을 다시 구현하지 않는다.
"""
...
class ServiceMemoryAccess:
    ...
    async def search(self, query: str, **kwargs: Any) -> list[Any]:
        ...
        entries = {
            ...
            for entry in self.service.read(self.token, alias, limit=kwargs.pop("scan", 500))
        }
