...
# 컨테이너 안에서 자기 Control API 를 부른다 — 배포된 에이전트의 포트는 호스트에 열려 있지 않다
INVOKE = """
...
print((result.get("error") or {}).get("code") or result["status"])
"""
HEALTH = """
...
def test_a_session_that_cannot_reconnect_makes_the_agent_unhealthy_and_restarted(plane):
    ...
    # 재연결을 시도한다
    outcome = run_in(
        RESEARCHER, INVOKE, "e2e-mcp-down", 'use tool mcp__tools__echo with {"text": "hi"}'
    )

    assert outcome.startswith("MCP_"), outcome
    assert run_in(RESEARCHER, HEALTH) == "unhealthy"
