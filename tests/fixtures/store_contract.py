"""Memory store contract tests, shared by every backend.

두 백엔드가 **같은 계약**을 지키는지 한 벌의 테스트로 검증한다. 백엔드별
파일이 이 모듈의 테스트를 가져다 쓰고, ``store`` fixture 만 각자 제공한다 —
계약 테스트가 갈라지면 백엔드 교체가 조용히 계약을 깬다.
"""
